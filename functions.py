...
def RequestService(sender, password, mode, value):
    ...
    Kc_tgs = BinaryToByte(DecryptAES(BinaryToByte(message_A), password_hash.encode(), BinaryToByte(nonce_A)))
    message_C = StringToBinary(f'{mode}||{value}')
    message_D, nonce_D = EncryptAES(StringToBinary(sender), Kc_tgs)
    ...
    if content_TGS == 'Denied':
        ...
        if mode == '0':
            ...
            message_E = segment_TGS[0]
            nonce_E = segment_TGS[1]
            plain_E = BinaryToString(DecryptAES(BinaryToByte(message_E), Kc_tgs, BinaryToByte(nonce_E))).split(',')
            PU = plain_E[0]
            n = plain_E[1]
            return PU, n
        elif mode == '1':
